    ...
    <div className="h-screen w-full flex items-center justify-center bg-transparent font-sans p-4 overflow-hidden">
      ...
      <div className="relative w-full max-w-[850px] h-[520px] bg-white rounded-[2.5rem] shadow-[0_20px_60px_rgba(0,0,0,0.15)] overflow-hidden flex">
        ...
        >
          ...
          <h2 className="text-3xl font-bold mb-4">{isSignIn ? "Hello, Friend!" : "Welcome Back!"}</h2>
          <p className="text-sm mb-10 opacity-90 leading-relaxed px-4">
            {isSignIn ? "Begin your journey with O Dental Clinic for a brighter smile." : "Please login with your personal info to stay connected."}
          </p>
          <button onClick={toggleView} className="px-12 py-3 border-2 border-white rounded-full font-bold hover:bg-white hover:text-[#3a5ed4] transition-all cursor-pointer">
        ...
        <div className="relative w-full flex h-full">
          ...
          <div className={`absolute left-0 w-full md:w-1/2 h-full flex flex-col items-center justify-center p-8 transition-all duration-500 ${!isSignIn ? "opacity-0 invisible" : "opacity-100 visible"}`}>
            ...
            <form className="w-full max-w-xs space-y-4" onSubmit={handleSubmit}>
              ...
              <button type="submit" className="w-full bg-[#3a5ed4] text-white py-3.5 rounded-full font-bold shadow-lg hover:bg-blue-700 transition-all cursor-pointer active:scale-95">SIGN IN</button>
            </form>
          </div>
          ...
          <div className={`absolute right-0 w-full md:w-1/2 h-full flex flex-col items-center justify-center p-8 transition-all duration-500 ${isSignIn ? "opacity-0 invisible" : "opacity-100 visible"}`}>
            ...
            <form className="w-full max-w-xs space-y-4" onSubmit={handleSubmit}>
              ...
              <InputGroup icon={<Mail className="w-4 h-4" />} type="email" placeholder="Email" required />
              <InputGroup icon={<Lock className="w-4 h-4" />} type="password" placeholder="Password" required />
              <button type="submit" className="w-full bg-[#3a5ed4] text-white py-3.5 rounded-full font-bold shadow-lg hover:bg-blue-700 transition-all cursor-pointer active:scale-95">SIGN UP</button>
            </form>
          </div>
        </div>
      </div>
...
const SocialButton = ({ icon }) => (
  <button className="p-3 border border-gray-100 rounded-2xl hover:bg-gray-50 transition-all cursor-pointer">{icon}</button>
);

const InputGroup = ({ icon, ...props }) => (
  <div className="relative">
    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400">{icon}</div>
    <input {...props} className="w-full pl-12 pr-4 py-3.5 bg-[#eef0f7] border-none rounded-2xl text-sm outline-none focus:ring-2 focus:ring-[#3a5ed4] transition-all" />
  </div>
);
